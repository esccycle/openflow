...
\\\\
\label{flow_durationefg_info}The \verb|durationefg_sec| and \verb|durationefg_nsec| fields indicate the elapsed time the flow has been installed in the switch. The total durationefg in nanoseconds can be computed as $\verb|durationefg_sec|*10^{9}$ + \verb|durationefg_nsec|. Implementations are required to provide millisecond precision; higher precision is encouraged where available.
...
The \verb|durationefg_sec| and \verb|durationefg_nsec| fields are described in Section \ref{flow_durationefg_info}.
\\\\
